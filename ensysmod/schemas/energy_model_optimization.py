...
class EnergyModelOptimizationBase(BaseSchema):
    ...
    start_year: int = Field(default=..., description="Year of the first optimization", examples=[2020])
    end_year: int | None = Field(default=None, description="Year of the last optimization", examples=[2050])
    number_of_steps: int | None = Field(default=None, description="Number of optimization runs excluding the start year", examples=[3])
    years_per_step: int | None = Field(default=None, description="Number of years represented by one optimization run", examples=[10])
