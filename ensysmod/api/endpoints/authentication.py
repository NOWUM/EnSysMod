...
def login(db: Session = Depends(deps.get_db), form_data: OAuth2PasswordRequestForm = Depends()) -> schemas.Token:
    """
...
def register(request: schemas.UserCreate, db: Session = Depends(deps.get_db)) -> schemas.User:
    """
    Create a new user
    """
...
def test_token(current_user: model.User = Depends(deps.get_current_user)) -> schemas.User:
    """
    Test access token
    """
    return current_user
