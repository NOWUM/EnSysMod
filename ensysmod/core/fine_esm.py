...
def generate_esm_from_model(db: Session, model: EnergyModel) -> EnergySystemModel:
    ...
    for source in model.dataset.sources:
        add_source(esM, db, source, region_ids)

    # Add all sinks
    for sink in model.dataset.sinks:
        add_sink(esM, db, sink, region_ids)

    # Add all conversions
    for conversion in model.dataset.conversions:
        add_conversion(esM, db, conversion, region_ids)

    # Add all storages
    for storage in model.dataset.storages:
        add_storage(esM, db, storage, region_ids)

    # Add all transmissions
    for transmission in model.dataset.transmissions:
        add_transmission(esM, db, transmission, region_ids)

    return esM


def add_source(esM: EnergySystemModel, db: Session, source: EnergySource, region_ids: List[int]) -> None:
...
def add_sink(esM: EnergySystemModel, db: Session, sink: EnergySink, region_ids: List[int]) -> None:
...
def add_conversion(esM: EnergySystemModel, db: Session, conversion: EnergyConversion, region_ids: List[int]) -> None:
...
def add_storage(esM: EnergySystemModel, db: Session, storage: EnergyStorage, region_ids: List[int]) -> None:
...
def add_transmission(esM: EnergySystemModel, db: Session,
                     transmission: EnergyTransmission, region_ids: List[int]) -> None:
