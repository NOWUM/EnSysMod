...
def test_error_on_negativ_invest_per_capacity(schema: Type[BaseModel], data: Dict[str, Any]):
    """
    Test that a invest per capacity is not negativ 
    """
    ...
    assert exc_info.value.errors()[0]["msg"] == "Invest per capacity must be zero or positiv."
    assert exc_info.value.errors()[0]["type"] == "value_error"



@pytest.mark.parametrize("schema,data", schemas_with_invest_per_capacity)
def test_ok_invest_per_capacitys(schema: Type[BaseModel], data: Dict[str, Any]):
    """
    Test that a invest per capacity with everything over 0 and 0 is valid
    """
