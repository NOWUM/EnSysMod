...
def random_region_create(db: Session) -> RegionCreate:
    ...
    return RegionCreate(name=f"Region-{dataset.id}-{random_lower_string()}", ref_dataset=dataset.id)
...
def fixed_region_create(db: Session) -> RegionCreate:
    ...
    return RegionCreate(name=f"Region-{dataset.id}-Fixed", ref_dataset=dataset.id)


def fixed_region_alternative_create(db: Session) -> RegionCreate:
    dataset = fixed_existing_dataset(db)
    return RegionCreate(name=f"Region-{dataset.id}-Fixed-alternative", ref_dataset=dataset.id)
...
def fixed_existing_region_alternative(db: Session) -> Region:
    create_request = fixed_region_alternative_create(db)
    region = crud.region.get_by_dataset_and_name(db=db, dataset_id=create_request.ref_dataset, name=create_request.name)
    if region is None:
        region = crud.region.create(db=db, obj_in=create_request)
    return region
